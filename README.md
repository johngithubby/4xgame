...
- The minigame renders a three-person squad from the optimized 59,288-triangle licensed SWAT model with shared albedo/normal PBR materials and Mixamo-retargeted lowered-rifle idle/run clips. The navy commander, charcoal/burgundy scout, and dark-olive heavy use different removable equipment layers and per-instance colours so they do not read as replicas.
- Generated survivor rigs remain as hidden compatibility skeletons, while each visible imported SWAT model owns the actual barrel anchor used for aiming and tracer origins.
- Both legacy soldier decals stay disabled after the full 3D models load; the chase camera sees the imported Humanoid skins, their real rigid rifles, and their distinct removable-equipment silhouettes from behind.
...
- All three survivors crossfade between Mixamo's complete lowered-rifle idle and looping in-place `Walk With Rifle` on fully weighted layers. Stable one-third-cycle offsets keep the commander, scout, and heavy out of lockstep without changing playback speed or causing foot sliding; zombies use a slower in-place shamble.
...
- Current EditMode test coverage includes Phase 1 gameplay rules, three imported survivor weapon/muzzle rotation, shared PBR texture/normal binding, distinct dark wardrobe and equipment signatures, Animator controllers, procedural/imported-bone isolation, muzzle-origin and detailed muzzle-transform shot events, Phase 2 progression/save/mission completion rules, base-building timers, the local daily objective, advanced missions, and Phase 3 hero inventory rules.
