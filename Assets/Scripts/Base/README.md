...
- `BaseSceneBootstrap.cs`: Builds the Phase 2 base scene at runtime, loads local progress, completes ready HQ upgrades, grants HQ milestone hero rewards, lays out a phone-sized HUD with anchor-matched text pivots so edge labels stay inside the viewport, saves mission selection changes, shows base feedback messages, and navigates to the minigame or Hero scene.
