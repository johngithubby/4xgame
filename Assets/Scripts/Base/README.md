...
- `BaseSceneBootstrap.cs`: Builds the Phase 2 base scene at runtime, loads local progress, rolls over the local daily objective, completes ready HQ upgrades, grants HQ milestone hero rewards, lays out a phone-sized HUD with anchor-matched text pivots so edge labels stay inside the viewport, saves direct mission button selection and daily objective claim changes, shows base feedback messages, and navigates to the minigame or Hero scene.
