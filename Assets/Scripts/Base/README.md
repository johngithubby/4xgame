...
The base scene is local-only. It lets the player collect coins, spend coins on a timed HQ upgrade, persist progress locally, see the minigame win reward hint, reset local progress in editor/development builds, and launch the minigame with an HQ-derived starting squad bonus. The HUD uses a narrow mobile reference resolution so the placeholder controls fit better on iPhone-like screens.
