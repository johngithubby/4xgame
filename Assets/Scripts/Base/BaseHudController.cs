...
namespace LaneSurvivor.Base
{
    public sealed class BaseHudController : MonoBehaviour
    {
        ...
        public void UpdateView(SaveGameData saveData, int remainingSeconds, int bioLabRemainingSeconds, int hangarRemainingSeconds, int trainingRemainingSeconds, int livingQuartersRemainingSeconds, string statusOverride)
        {
            ...
            int livingQuartersLevel = Mathf.Max(1, saveData?.livingQuartersLevel ?? 1);
            ...
            bool livingQuartersUpgradeRunning = saveData != null && saveData.livingQuartersUpgradeInProgress;
                ...
                : bioLabUpgradeRunning
                    ...
                    : hangarUpgradeRunning
                        ...
                        : trainingUpgradeRunning
                            ...
                            : livingQuartersUpgradeRunning
                                ? "Living quarters upgrade in progress"
                                : $"Next HQ upgrade: {upgradeCost} coins";

            titleText.text = "Base";
            string upgradeStatus = upgradeRunning
                ? $"HQ Upgrade: {remainingSeconds}s"
                : canAffordUpgrade
                    ? $"HQ Upgrade: Ready ({upgradeCost}c)"
                    : $"HQ Upgrade: Need {upgradeCost}c";
            ...
            string livingQuartersStatus = livingQuartersUpgradeRunning ? $"LQ Upgrade: {livingQuartersRemainingSeconds}s" : "LQ Upgrade: Ready";
            creditsButtonText.text = $"Credits: {coins}";
            creditsDetailText.text = BuildCreditsDetailText(coins, hqLevel, bioLabLevel, hangarLevel, trainingFacilityLevel, livingQuartersLevel, upgradeStatus, bioLabStatus, hangarStatus, trainingStatus, livingQuartersStatus);
            ...
            // Keep the HQ button clickable while idle so unaffordable taps can explain the needed credits.
            upgradeButton.interactable = !upgradeRunning;
        ...
        private static string BuildCreditsDetailText(int coins, int hqLevel, int bioLabLevel, int hangarLevel, int trainingFacilityLevel, int livingQuartersLevel, string upgradeStatus, string bioLabStatus, string hangarStatus, string trainingStatus, string livingQuartersStatus)
        {
            // Keep the expanded panel short so it replaces the old left text stack without becoming another wall.
            return $"Coins: {coins}\nHQ Level: {hqLevel}\nBio Lab: {bioLabLevel}\nHangar: {hangarLevel}\nTraining: {trainingFacilityLevel}\nLiving Qtrs: {livingQuartersLevel}\n{upgradeStatus}\n{bioLabStatus}\n{hangarStatus}\n{trainingStatus}\n{livingQuartersStatus}";
        }
