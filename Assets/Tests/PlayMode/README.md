...
- `BaseSceneFlowTests.cs`: Loads runtime-built scenes, verifies Base collect/upgrade persistence, mission selection persistence, completed-upgrade feedback, invokes Base-to-Minigame and Base-to-Heroes navigation, checks Minigame start movement, lane buttons, restart, rewards, mission unlocks, and end-screen return to Base, and checks Hero screen level-up/equip actions persist local hero state.
